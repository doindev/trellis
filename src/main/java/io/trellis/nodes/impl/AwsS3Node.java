...
public class AwsS3Node extends AbstractApiNode {
	...
	private void addBucketParameters(List<NodeParameter> params) {
		...
		// Create: region
		params.add(NodeParameter.builder()
			.name("region").displayName("Region").type(ParameterType.OPTIONS).defaultValue("us-east-1")
			.displayOptions(Map.of("show", Map.of("resource", List.of("bucket"), "operation", List.of("create"))))
			.options(List.of(
				ParameterOption.builder().name("US East (N. Virginia)").value("us-east-1").build(),
				ParameterOption.builder().name("US East (Ohio)").value("us-east-2").build(),
				ParameterOption.builder().name("US West (N. California)").value("us-west-1").build(),
				ParameterOption.builder().name("US West (Oregon)").value("us-west-2").build(),
				ParameterOption.builder().name("EU (Ireland)").value("eu-west-1").build(),
				ParameterOption.builder().name("EU (Frankfurt)").value("eu-central-1").build(),
				ParameterOption.builder().name("Asia Pacific (Tokyo)").value("ap-northeast-1").build(),
				ParameterOption.builder().name("Asia Pacific (Sydney)").value("ap-southeast-2").build()
			)).build());
	}
	...
	public NodeExecutionResult execute(NodeExecutionContext context) {
		...
		try {
			...
			return handleError(context, "AWS S3 API error: " + e.getMessage(), e);
		}
	}
	...
	private NodeExecutionResult executeBucket(NodeExecutionContext context, Map<String, Object> credentials) throws Exception {
		...
		String region = getRegion(credentials);

		switch (operation) {
			case "create": {
				String bucketName = context.getParameter("bucketName", "");
				String bucketRegion = context.getParameter("region", region);

				String url = getS3Url(bucketName, "", region);

				Map<String, Object> body = new LinkedHashMap<>();
				if (!"us-east-1".equals(bucketRegion)) {
					body.put("CreateBucketConfiguration", Map.of("LocationConstraint", bucketRegion));
				}

				HttpResponse<String> response = put(url, body.isEmpty() ? "" : toJson(body), headers);
			...
			case "getAll": {
				String url = getS3ServiceUrl(region);
	...
	private NodeExecutionResult executeObject(NodeExecutionContext context, Map<String, Object> credentials) throws Exception {
		...
		switch (operation) {
			case "copy": {
				...
				String url = getS3Url(destBucket, destKey, region);
			...
			case "delete": {
				String bucket = context.getParameter("objectBucket", "");
				String key = context.getParameter("objectKey", "");
				String url = getS3Url(bucket, key, region);
				HttpResponse<String> response = delete(url, headers);
				return toDeleteResult(response);
			}
			case "download": {
			...
			case "getAll": {
				...
				String url = getS3Url(bucket, "", region) + "?list-type=2&max-keys=" + maxKeys;
	...
	private Map<String, String> getAuthHeaders(Map<String, Object> credentials) {
		...
		// AWS Signature V4 would typically be computed here; for REST-based calls
		// we pass credentials so the platform can sign the request
	...
	private String getRegion(Map<String, Object> credentials) {
		return String.valueOf(credentials.getOrDefault("region", DEFAULT_REGION));
	}

	private String getS3Url(String bucket, String key, String region) {
	...
	private String getS3ServiceUrl(String region) {
	...
	private NodeExecutionResult s3Error(HttpResponse<String> response) {
		...
		return NodeExecutionResult.error("AWS S3 API error (HTTP " + response.statusCode() + "): " + body);
	}
}
