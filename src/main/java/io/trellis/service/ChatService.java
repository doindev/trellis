...
import io.trellis.dto.ChatMessageResponse;
...
public class ChatService {
    ...
    private static final String DEFAULT_SYSTEM_PROMPT = "You are a helpful workflow automation assistant for Trellis, " +
    ...
    public ChatMessageResponse sendMessage(String sessionId, String content) {
        ChatSessionEntity session = chatSessionRepository.findById(sessionId)
                .orElseThrow(() -> new io.trellis.exception.NotFoundException("Chat session not found: " + sessionId));
        ...
        if (streamingModel.isPresent()) {
            ...
            saveAndSend(sessionId, "AI chat is not configured. Set `trellis.ai.openai.api-key` in application.properties to enable it.");
        }
