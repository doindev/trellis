...
const CwcNode = memo(({ id, data, selected }: NodeProps & { data: CwcNodeData }) => {
  ...
  const nodeContent = (
    <div className={`cwc-node action-node${statusClass}${selectedClass}${disabledClass}${hasAiHandles ? ' has-ai-handles' : ''}${isIconOnly ? ' icon-only' : ''}`} style={nodeStyle}>
            ...
            return (
              <div key={`ai-out-${output.name}`} className="ai-handle-wrapper">
                <Handle
                  ...
                  className="cwc-handle ai-handle"
      ...
      {mainOutputs.map((output, index) => (
        <React.Fragment key={`output-${output.name}`}>
          ...
          />
          {(data.nodeType === 'if' && mainOutputs.length >= 2 || data.nodeType === 'switch' || data.nodeType === 'loopOverItems' || data.nodeType === 'compareDatasets' || data.nodeType === 'removeDuplicates' || data.nodeType === 'guardrails' || data.nodeType === 'filter' || data.nodeType === 'schemaValidator' || data.nodeType === 'tryCatch' || data.nodeType === 'retry' || data.nodeType === 'cache') && (
            <span
...
CwcNode.displayName = 'CwcNode';
export default CwcNode;
